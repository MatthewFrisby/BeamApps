...
router.get('/lasercutter', function(req, res, next) {
	LaserCutter.find({},{create_date: 1, location: 1},function(err, lasercutter) {
		if (err) {
			res.json({
				status: "error",
				message: err,
...
router.get('/lasercutter/:location', function(req, res, next) {
    ...
    LaserCutter.find({location: req.params.location},{create_date: 1, location: 1}, function(err, lasercutter){
      if (err) {
        res.json({
          status: "error",
          message: err,
...
router.get('/lasercutter/admin/:location', function(req, res, next) {
					...
					LaserCutter.find({location: req.params.location}, function(err, lasercutter){
			      if (err) {
			        res.json({
			          status: "error",
			          message: err,
...
router.put('/lasercutter/admin/:location', function(req, res, next) {
