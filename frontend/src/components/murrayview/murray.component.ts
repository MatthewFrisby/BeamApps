...
@Component({
  selector: 'murray-root',
  templateUrl: './murray.component.html',
  providers: [LaserCutterService],
})


export class Murray implements OnInit {
  ...
  ngOnInit() {
    ...
    this.lasercutter.getQueueAtLocationAdmin("Murray").subscribe(data => { this.murrayQueue = data.data,  this.newDate(this.murrayQueue), this.murrayViewO = this.murrayQueue.slice(0,3), this.murrayViewW = this.murrayQueue.slice(3,)});
